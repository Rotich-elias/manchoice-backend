...
<div class="mb-6 flex items-center justify-between">
    ...
    <div class="flex items-center space-x-2">
        ...
        @endif
        <a href="/admin/reports/payments?format=pdf{{ isset($currentStatus) && $currentStatus !== 'all' ? '&status=' . $currentStatus : '' }}" class="bg-red-600 hover:bg-red-700 text-white px-3 py-2 rounded flex items-center">
            <svg class="w-5 h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path>
            </svg>
            PDF
        </a>
        <a href="/admin/reports/payments?format=excel{{ isset($currentStatus) && $currentStatus !== 'all' ? '&status=' . $currentStatus : '' }}" class="bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded flex items-center">
            <svg class="w-5 h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="mb-4">
    ...
    <div class="bg-white p-4 rounded-lg shadow">
        <form method="GET" action="/admin/payments" class="flex gap-2">
            @if(request('status'))
            <input type="hidden" name="status" value="{{ request('status') }}">
            @endif
            <input type="text" name="search" value="{{ request('search') }}"
                   placeholder="Search by Transaction ID, Customer Name, Loan #, or Receipt"
                   class="flex-1 px-4 py-2 border rounded-lg">
            <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white px-6 py-2 rounded-lg">Search</button>
            @if(request('search'))
            <a href="/admin/payments{{ request('status') ? '?status='.request('status') : '' }}"
